...
parser.add_argument('--save_freq',default=1e3, help='seed of the games')
...
class Smoothing_DQN(object):
    def __init__(self):
        ...
        self.optimizer1 = torch.optim.Adam(self.pred_net_Q2.parameters(), lr=LR)
...
dqn = Smoothing_DQN()
logdir = './DOUBLE_average_choose_DQN/%s' % args.games + '/%i' % int(time.time())

logger_kwargs = setup_logger_kwargs(args.games+"DOUBLE_average_choose", args.seed, data_dir=logdir)
